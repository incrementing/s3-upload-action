...
| `content-disposition`   | Specify the contents of the 'Content-Disposition' header when downloading the file, eg `attachment; filename="image.png"`.                                                                                                                                         |                                   |
| `tags`                  | Comma-separated list of tags to apply to the uploaded file, eg `tag1=value1,tag2=value2`.                                                                                                                                                                          |                                   |
...
| `checksum-algorithm`    | Algorithm to use for the file checksum. Supported algorithms are `CRC32`, `CRC32C`, `SHA1`, and `SHA256`. If not specified, AWS SDK defaults to `CRC32`.                                                                                                           | `CRC32`                           |
| `checksum`              | Precomputed checksum value of the file. If not specified, the checksum is calculated automatically based on the specified `checksum-algorithm`. If specified, AWS will reject the PutObject if the provided checksum does not match the checksum calculated by AWS |                                   |
