...
if (NODE_ENV != "local") {
	input = {
		...
		awsEndpointUrl: core.getInput("aws-endpoint-url"),
...
const s3 = new S3({
	...
	endpoint: input.awsEndpointUrl || undefined,
});
...
async function run(input) {
	...
	if (input.checksumAlgorithm !== "") {
		if (
			!["CRC32", "CRC32C", "CRC64NVME", "SHA1", "SHA256"].includes(
				input.checksumAlgorithm,
			)
		) {
			throw new Error(
				`Unsupported checksum algorithm: ${input.checksumAlgorithm}`,
			);
		}
		...
		if (input.public == "true") {
			...
			if (input.alternativeDomainPublic) {
				fileUrl = fileUrl.replace(
					`${input.awsBucket}.s3.${input.awsRegion}.amazonaws.com/${bucketRoot}`,
					`${input.alternativeDomainPublic}/`,
				);
			}
		} else {
			...
			if (input.alternativeDomainPrivate) {
				fileUrl = fileUrl.replace(
					`${input.awsBucket}.s3.${input.awsRegion}.amazonaws.com/${bucketRoot}`,
					`${input.alternativeDomainPrivate}/`,
				);
			}
		}
